...
def readTensoIRInfo(path, white_background, eval):
    ...
    def load_from_dir(target_dir, split_prefix):
        cam_infos = []
        if not os.path.exists(target_dir):
            return cam_infos
            
        subdirs = sorted([d for d in os.listdir(target_dir) if os.path.isdir(os.path.join(target_dir, d)) and (split_prefix in d or d.isdigit())])
        for idx, d in enumerate(subdirs):
            d_path = os.path.join(target_dir, d)
            
            # Find any json file in the directory (usually metadata.json)
            json_files = [f for f in os.listdir(d_path) if f.endswith('.json')]
            if len(json_files) == 0:
                continue
            metadata_path = os.path.join(d_path, json_files[0])
            ...
            if image_path is None:
                ...
                if len(filtered) > 0:
                    ...
                    # fallback to first png found
                    if len(all_pngs) > 0:
                        image_path = os.path.join(d_path, all_pngs[0])
                    else:
                        continue
            
            # Read image and apply white background if transparent
            image_name = f"{split_prefix}_{idx:03d}"
            image = Image.open(image_path)
            ...
            # Format 3: transforms.json style transform_matrix
                ...
                # fallback/unrecognized formats
                c2w = np.eye(4)
                
            # Convert from Blender/OpenGL (Y up, Z back) to COLMAP (Y down, Z forward)
            c2w[:3, 1:3] *= -1
            
            w2c_colmap = np.linalg.inv(c2w)
            R = np.transpose(w2c_colmap[:3, :3])
            T = w2c_colmap[:3, 3]
            ...
            if "cam_K" in metadata:
                ...
                # default focal length factor (typically 1.2 * width)
                fx = 1.2 * width
                fy = fx
    ...
    print("Reading TensoIR Training Cameras")
    train_cam_infos = load_from_dir(train_dir, "train")
    print("Reading TensoIR Test Cameras")
    test_cam_infos = load_from_dir(test_dir, "test")
    
    if len(train_cam_infos) == 0:
        # fallback to read from root folder with train prefix
        train_cam_infos = load_from_dir(path, "train")
    if len(test_cam_infos) == 0:
        test_cam_infos = load_from_dir(path, "test")
