...
class IDRTrainRunner():
    def __init__(self,**kwargs):
        ...
        if kwargs['is_continue'] and kwargs['timestamp'] == 'latest':
            exp_parent_dir = os.path.join('../', kwargs['exps_folder_name'], self.expname)
            if os.path.exists(exp_parent_dir):
                timestamps = sorted(os.listdir(exp_parent_dir), reverse=True)
                found_valid = False
                ckpt_target = str(kwargs['checkpoint']) + ".pth"
                for ts in timestamps:
                    model_ckpt = os.path.join(exp_parent_dir, ts, 'checkpoints', 'ModelParameters', ckpt_target)
                    if os.path.exists(model_ckpt):
                        timestamp = ts
                        found_valid = True
                        print(f"[Auto-Resume] Found latest valid checkpoint at: {model_ckpt}")
                        break
                if found_valid:
                    is_continue = True
                else:
                    print("[Auto-Resume] No valid checkpoint found in exps. Starting training from scratch.")
                    is_continue = False
                    timestamp = None
            else:
                is_continue = False
