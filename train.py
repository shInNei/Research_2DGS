...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint):
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        if iteration % 1000 == 0:
            print(f"\n[ITER {iteration}] Updating Visibility via BVH...")
            try:
                gaussians.update_visibility(sample_num=64)
                gaussians.finetune_visibility(iterations=500)
        ...
        with torch.no_grad():
            ...
            if iteration < opt.densify_until_iter:
                ...
                print(f"\n[ITER {iteration}] Final Densification Visibility Update...")
                try:
                    gaussians.update_visibility(sample_num=64)
